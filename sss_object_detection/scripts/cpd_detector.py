...
class CPDetector:
    ...
    def detect(self, ping):
        ...
        if rope:
            detections[ObjectID.ROPE] = {'pos': rope[0], 'confidence': rope[1]}
        if buoy:
            detections[ObjectID.BUOY] = {'pos': buoy[0], 'confidence': buoy[1]}
        return detections
    ...
    def _compare_region_with_surrounding(self, ping, bkps, window_size=50):
        ...
        post_window = ping[bkps[1]+1:min(bkps[1]+window_size, ping.shape[0])]
